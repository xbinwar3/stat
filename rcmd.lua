...
local adPars = {"type","act","v","channel","uid","p","ats"}
...
-- loop print ulog
local rec_type = ngx.unescape_uri(uri_args["type"])
local rec_act = ngx.unescape_uri(uri_args["act"])
local rec_ver = ngx.unescape_uri(uri_args["ver"])
...
local rec_group = ngx.now()
groupStr  = "&group="..rec_group
for k,v in pairs(values) do
   -- string replace
   item_arr = Split(values[k],":")
   itemStr = "&itemtype="..item_arr[1].."&id="..item_arr[2]
   local logStr = string.gsub(newArgs, ngx.unescape_uri(uri_args["v"]) , values[k])
   ...
   logStr = string.gsub(logStr, "ver=" , "ver2=")
   verStr = "&ver="..item_arr[3]
-- ngx.say(logStr..ipStr..groupStr..itemStr..verStr)
   ngx.location.capture('/i-rcmdlog?'..logStr..ipStr..groupStr..itemStr..verStr)
end
